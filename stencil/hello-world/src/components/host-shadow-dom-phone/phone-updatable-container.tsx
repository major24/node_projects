...
export class PhoneUpdatableContainer {
  ...
  @State() _displayList: Array<object> = [];
  ...
  createDisplayList(){
    let list = [];
    this._dataDisplay.map((d) => {
        list.push(<phone-display phone-type={d.description} phone-number={d.number}></phone-display>);
    });

    this._displayList = [...this._displayList, list];
  }
  ...
  // on update event from child component..
    ...
    onUpdatePhone(e) {
        ...
        // update display data to reflect the edit changes
        const changedData = JSON.parse(e.detail);
        let list = [];
        this._displayList = [];
        this._dataDisplay.map((dt) => {
            if (changedData.description == dt.description) {
                list.push(<phone-display phone-type={changedData.description} phone-number={changedData.number}></phone-display>);
            } else {
                list.push(<phone-display phone-type={dt.description} phone-number={dt.number}></phone-display>);
            }
        });
