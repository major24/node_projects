...
function applyPolyfills(window) {
  ...
  var promises = [];
  return window.customElements && (!window.Element || window.Element.prototype.closest && window.Element.prototype.matches && window.Element.prototype.remove) || promises.push(import('./polyfills/dom.js')), 
  ...
  window.fetch || promises.push(import('./polyfills/fetch.js')), 'undefined' != typeof WeakMap && window.CSS && window.CSS.supports && window.CSS.supports('color', 'var(--c)') || promises.push(import('./polyfills/css-shim.js')), 
  function checkIfURLIsSupported() {
    try {
      var u = new URL('b', 'http://a');
      return u.pathname = 'c%20d', 'http://a/c%20d' === u.href && u.searchParams;
    } catch (e) {
      return false;
    }
  } || promises.push(import('./polyfills/url.js')), Promise.all(promises).then(function(results) {
...
var createPlatformMain = function(namespace, Context, win, doc, resourcesUrl, hydratedCssClass, components) {
  var perf = win.performance;
  var cmpRegistry = {
    'html': {}
  };
  var App = win[namespace] = win[namespace] || {};
  var domApi = function(App, win, doc) {
  ...
  var defineComponent = function(cmpMeta, HostElementConstructor) {
    ...
    HostElementConstructor.observedAttributes = Object.values(cmpMeta.membersMeta).map(function(member) {
      return member.attribName;
    }).filter(function(attribName) {
      return !!attribName;
  ...
  // create the platform api which is used throughout common core code
    ...
    requestBundle: function(cmpMeta, elm, hmrVersionId) {
      ...
      cmpMeta.bundleIds(moduleOpts).then(function(cmpConstructor) {
        try {
          ...
          cmpMeta.componentConstructor = cmpConstructor, initStyleTemplate(0, cmpMeta, cmpMeta.encapsulationMeta, cmpConstructor.style, cmpConstructor.styleMode);
        } catch (e) {
  ...
  // set App Context
  return Context.isServer = Context.isPrerender = !(Context.isClient = true), Context.window = win, 
  Context.location = win.location, Context.document = doc, Context.resourcesUrl = Context.publicPath = resourcesUrl, 
  // add the h() fn to the app's global namespace
  App.h = h, App.Context = Context, 
  ...
  function(plt, App, win, apps, queuedComponentOnReadys, i) {
    if (
...
function defineCustomElement(win, cmpData, opts) {
  void 0 === opts && (opts = {});
  var cmpDataArray = Array.isArray(cmpData) ? cmpData : [ cmpData ];
  var doc = win.document;
  var hydratedCssClass = opts.hydratedCssClass || 'hydrated';
  var exclude = opts.exclude;
  exclude && (cmpDataArray = cmpDataArray.filter(function(c) {
    return -1 === exclude.indexOf(c[0]);
  }));
  var styleCmps = cmpDataArray.map(function(c) {
    return c[0];
  });
  if (styleCmps.length > 0) {
    ...
    styleElm.setAttribute('data-styles', ''), doc.head.insertBefore(styleElm, doc.head.firstChild);
  }
  var namespace = opts.namespace || 'mncomponent';
  return initCmpOnReady || (initCmpOnReady = true, function createComponentOnReadyPrototype(win, namespace, HTMLElementPrototype) {
    (win['s-apps'] = win['s-apps'] || []).push(namespace), HTMLElementPrototype.componentOnReady || (HTMLElementPrototype.componentOnReady = function componentOnReady() {
      /*tslint:disable*/
      var elm = this;
      function executor(resolve) {
        if (elm.nodeName.indexOf('-') > 0) {
          ...
          return void (win['s-cr'] = win['s-cr'] || []).push([ elm, resolve ]);
        }
  ...
  }(win, namespace, win.HTMLElement.prototype)), applyPolyfills(win).then(function() {
    if (!pltMap[namespace]) {
      var Context = {};
    ...
    function defineComponents() {
      ...
      cmpDataArray.forEach(function(c) {
        var HostElementConstructor;
        if (function isNative(fn) {
        ...
        // define the component as a custom element
                pltMap[namespace].defineComponent(function buildComponentLoader(c) {
          var meta = parseComponentLoader(c);
          var bundleIds = meta.bundleIds;
          var className = dashToPascalCase(c[0]);
          return meta.bundleIds = function(_a) {
            var mode = _a.mode, scoped = _a.scoped;
            return loadBundle('string' === typeof bundleIds ? bundleIds : bundleIds[mode], scoped, className);
          }, meta;
        }(c), HostElementConstructor);
      });
    }
    if (window.customStyleShim) return pltMap[namespace].customStyle = window.customStyleShim, 
...
function loadBundle(bundleId, useScopedCss, className) {
  ...
  /* webpackMode: "lazy" */
  './build/' + bundleId + (useScopedCss ? '.sc' : '') + '.entry.js').then(function(m) {
    return m[className];
  });
}
