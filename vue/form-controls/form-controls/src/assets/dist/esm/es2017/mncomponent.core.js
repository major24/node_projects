    ...
    requestBundle: (cmpMeta, elm, hmrVersionId) => {
      {
        ...
        cmpMeta.bundleIds(moduleOpts).then(cmpConstructor => {
          try {
            ...
            cmpMeta.componentConstructor = cmpConstructor, initStyleTemplate(domApi, cmpMeta, cmpMeta.encapsulationMeta, cmpConstructor.style, cmpConstructor.styleMode);
          } catch (e) {
...
function defineCustomElement(win, cmpData, opts = {}) {
  ...
  exclude && (cmpDataArray = cmpDataArray.filter(c => -1 === exclude.indexOf(c[0])));
  ...
  const namespace = opts.namespace || 'mncomponent';
  ...
  }(win, namespace, win.HTMLElement.prototype)), applyPolyfills(win).then(() => {
    if (!pltMap[namespace]) {
      const Context = {};
    ...
    function defineComponents() {
      ...
      cmpDataArray.forEach(c => {
        ...
        // define the component as a custom element
          ...
          return meta.bundleIds = (({mode: mode, scoped: scoped}) => {
